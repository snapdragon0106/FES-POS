 ...
 * Transactions table
 *
 * FIX: `items` must be a native JSON column, not varchar. Every other
 * part of the app (POSApp stats, HistoryTab, Dashboard, transaction.create
 * itself) reads/writes `items` as a plain JS array of objects, never
 * calling JSON.stringify/parse manually — that only works if Drizzle's
 * `json()` column type handles the (de)serialization transparently.
 * Defining it as varchar (as an earlier reconstruction of this file
 * mistakenly did) causes every checkout to fail at the database layer.
 */
